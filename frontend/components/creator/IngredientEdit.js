...
export default function IngredientEdit() {
    ...
    function addIngredient(event) {
        ...
        if (newOne.category === "") {
            setAlert("Please select a valid ingredient. If it is not in the databse, create it below.")
        } else {
            setAlert("");
            let newIngred = {
    ...
    return <div>
        ...
        <div className={`field control ${styles.querySearch}`}>
            <input onChange={(event) => {event.preventDefault(); clearCat(); dispatch(update_query(event.target.value));}} id={searchId} className="input" placeholder="Search item"/>
        ...
        <form onSubmit={addIngredient} autocomplete="false">
            <div className="control">
                <div className="field control">
                    <input required={true} onChange={updateIng} className="input" type="number" placeholder="Quantity" name="qty"/>
                </div>
                <div className="field control">
                    <input required={true} onChange={updateIng} className="input" type="text" placeholder="Adjective" name="adj"/>
                </div>
                <div className="field control">
                    <input required={true} onChange={updateIng} className="input" type="text" placeholder="Unit" name="unit"/>
                </div>
                <div className="field control">
                ...
                <div className="field control">
                    <button onClick={(e) => {e.preventDefault(); setNewOne(initalIngredient)}} style={{width:"100%"}} className="button">
                    Clear
                    </button>
                </div>
                <div className="field control">
